# Data dictionary — approved analytical dataset, 2026-09-06

The implemented panel is `data/processed/approved/crushed_stone_state_year.csv`: **450 state-year rows and 65 fields**. The [approved machine-readable dictionary](approved_variable_dictionary.csv) covers every field, including source flags, price adjustments and exact-calendar lags. The [approved profile](../output/tables/approved_variable_profile.csv) records types, missingness and ranges; the [quality report](data_quality_report.md) documents eligibility and anomalies.

The original 80-column panel is preserved separately. Its [legacy dictionary](variable_dictionary.csv), [baseline profile](evidence/variable_profile.csv), and [Gate 1 dictionary](evidence/data_dictionary_gate1.md) describe the inherited files. The mappings below connect those legacy names to implemented analytical concepts; statements about what was originally stored refer to that preserved panel.

The approved pipeline repairs missing CPI support and FHWA timing, builds lags by exact calendar self-joins, and corrects MSHA parsing. MSHA secondary quantities changed in 256 state-years; legacy values and legacy reconciliation do not supersede the [corrected parsing evidence](evidence/phase2_parser_correction.md). Target suppression remains missing; no target imputation was applied. Model roles and final recommendations are in [Decision Gate 2](decision_gate_2.md).

## Keys, target and primary candidates

| Existing field(s) | Approved analytical name | Definition / units | Missingness / handling |
|---|---|---|---|
...
| Three primary predictors at `t−1` (not stored) | corresponding `lag1_` fields | Prior-year units, outlays and construction GDP | Build from eligible source values before joining target year. Masked FHWA propagates to next target year. Strict sample 371. |
...
| `inflation_pct` | Annual-mean CPI growth, percent | Stored panel missing all 50 copies of 2015. The approved pipeline includes verified 2014 CPI and has no missing 2015 inflation values. |
...
The machine-readable approved dictionary defines the implemented fields. [Methodology decisions](methodology_decisions.md), [Gate 1](decision_gate_1.md), and the [Gate 2 model register](../output/tables/model_results_register.csv) distinguish approved design, evaluated features and pending final-model approval.
